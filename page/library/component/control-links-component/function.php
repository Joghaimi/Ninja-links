...
if(isset($_POST['add'])){
    ...
    $stmt='INSERT INTO control_sec_link(id,name,link,about,types) values(:id,:name,:url,:about,:types)';
    $param=['id'=>$link_to_print,':name'=>$sub_name,':url'=>$url,':about'=>$about,':types'=>$type];
    if(querys::query($stmt,$param)){
    header("LOCATION:control_links.php?power_id=$link_to_print");
    }
}
/**new delete link */
if(isset($_POST['delete'])){
    if(isset($_POST['delet_id'])){
        $id=$_POST['delet_id'];
        $link_to_print=prot::valid($_GET['power_id']);        
        $stmt='DELETE FROM control_sec_link WHERE ids=:ids';
        $param=[':ids'=>$id];
        if(querys::query($stmt,$param)){
            header("LOCATION:control_links.php?power_id=$link_to_print");
